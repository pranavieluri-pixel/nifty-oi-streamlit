...
# Rocket / thinking logic
rocket_symbol = "⚪️"
if total_pcr > 1 and atm_ce_risk > atm_pe_risk:
    rocket_symbol = "🚀"  # Strong bullish
elif total_pcr < 1 and atm_pe_risk > atm_ce_risk:
    rocket_symbol = "🔥"  # Strong bearish
elif (total_pcr > 1 and atm_ce_risk < atm_pe_risk) or (total_pcr < 1 and atm_pe_risk < atm_ce_risk):
    rocket_symbol = "🤔"  # Conflict / wait
else:
    rocket_symbol = "⚪️"  # Neutral
...
show_cols = [
    ...
    "strikeLabel",
    "PE_LTP","PE_pchgOI","PE_Risk","PE_OI"
]
...
def style_row(row):
    ...
    for col in ["CE_Risk", "PE_Risk"]:
        val = int(row[col])
        if val > 0:
            styles[col_idx[col]] += '; color: green'
        elif val < 0:
            styles[col_idx[col]] += '; color: red'
        else:
            styles[col_idx[col]] += '; color: black'
...
# ----------------- Bottom ticker -----------------
st.markdown("---")
st.markdown(f"**Live Snapshot:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Spot: {safe_int(spot_price)} | "
            f"PCR (all shown): {int(round(total_pcr)) if total_pcr!=float('inf') else '∞'} → {trend} | "
            f"PCR (ATM ±4): {int(round(atm_pcr)) if atm_pcr!=float('inf') else '∞'} → {atm_trend} | {rocket_symbol}")
